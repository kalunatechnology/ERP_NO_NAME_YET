  ...
  return (
    ...
    >
      <div className="pb-3 border-b border-gray-100">
        <h3 className="text-base sm:text-lg font-bold text-[#0E341F]">Number of Projects</h3>
        <p className="text-xs text-[#637566] mt-0.5 font-medium">Overall Portfolio Distribution</p>
      </div>

      <div className="grid grid-cols-[100px_1fr] sm:grid-cols-[110px_1fr] items-center gap-4 my-auto py-3">
        {/* SVG Donut */}
          ...
          {chartData.map((item) => (
            <div key={item.label} className="flex items-center justify-between text-xs gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-[#4A5D4E] font-medium truncate">{item.label}</span>
              </div>
