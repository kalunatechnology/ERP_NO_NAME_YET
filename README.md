...
> **Current implementation (7 September 2026):** the production path is `backend-express/` + `frontend-next/`, not the legacy Django backend described later in this historical document. The complete current baseline, route coverage, loading architecture, cache behavior, and verified benchmarks are in [Current Implementation Status](docs/CURRENT_IMPLEMENTATION_STATUS.md). Detailed technical truth remains in [System Documentation](docs/SYSTEM_DOCUMENTATION.md) and [Database Documentation](docs/DATABASE_DOCUMENTATION.md).

> Current verification: 2,616/2,616 routes registered, 783/783 authenticated GET routes passed, 1,833/1,833 mutation pipelines passed safely, nine login cases stayed within 3 seconds, and the latest login-to-complete-dashboard benchmark was 2,009 ms. Historical sections below are retained for migration traceability and must not be used as the active runtime runbook.

# Legacy Django Backend — Historical Technical Documentation
