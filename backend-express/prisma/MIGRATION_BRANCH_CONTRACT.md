...
6. **Application build and database migration are separate operations.** `npm run build` must never connect to or mutate a live database. The production Hostinger release command, `npm run deploy:hostinger`, runs the guarded migration gate first and only compiles when migration succeeds.
...
2. For every Hostinger production release, configure the build command as `npm run deploy:hostinger`. It runs the guarded database release before compiling the application.
...
A normal application build does **not** run either seed profile or database migration. The production Hostinger deployment command runs only the guarded schema migration; it never runs seeds or overwrites business data.
