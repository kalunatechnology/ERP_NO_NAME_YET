...
**Perubahan terakhir:** `ACC-2026-09-16-03` — Audit regresi login Q10 pasca-deploy dan uji latensi produksi.
...
- Mapping route aktif: `/tasks` dan `/projects` → `PROJECTS`; `/crm` → `CRM`; `/finance` → `FINANCE`; `/reporting` → `REPORTING`; `/resources` → `ANALYTICS`; `/administration` hanya untuk Super Admin atau Company Admin tanpa entitlement modul operasional; `/dashboard` tidak memiliki satu module karena section BFF dinilai satu per satu.
...
|---|---|---|---|---|
| `ACC-2026-09-16-03` | 16 Sep 2026 | Production login measured; intermittent screenshot not reproduced | Diff commit terakhir `5946526` terhadap `34007b8` tidak mengubah auth, Axios, middleware login, atau mount auth. Benchmark API produksi dengan akun demo Super Admin: 1 run 1.898 ms dan 9 run berikutnya 9/9 PASS dalam 327–1.170 ms, durasi auth server 224–324 ms pada seri sembilan. Login tambahan 1.757 ms diikuti profil `/auth/me/` 200 dalam 1.346 ms dengan identitas Super Admin benar. Ini membuktikan target Q10 pada kondisi produksi saat pengukuran, bukan menjamin cold restart atau insiden sesaat. Kenaikan timeout FE ke 30 detik dibatalkan karena menutupi SLA. Startup backend kini menunggu query SQL nyata setelah `$connect` sebelum membuka port; hal ini menutup readiness palsu setelah pooler wake, tetapi belum dapat membuktikan SLA cold restart tanpa deploy baru. Benchmark berikutnya memerlukan URL target eksplisit dan tidak dicampurkan ke build otomatis karena login memperbarui `last_login_at`. | `server.ts`, `benchmark_demo_login.js`, git diff `34007b8..5946526`, `auth.api.ts`, Q11 |
| `ACC-2026-09-16-02` | 16 Sep 2026 | Implemented locally; production smoke pending | Request login `/auth/token/` tidak lagi membawa bearer token, company header, atau idempotency key dari sesi lama. Timeout FE tetap 15 detik, tetapi pesan kegagalan tidak menyalahkan password/role; target healthy-path Q10 tetap 3 detik. Backend mencatat durasi lookup user, verifikasi password, snapshot akses, dan total login bila lambat tanpa membocorkan identifier. Pemeriksaan produksi read-only menemukan `/health` 200, preflight CORS 204, login identifier fiktif 401 sekitar 2 detik, serta akun Super Admin aktif/role sesuai; lookup dari koneksi DB lokal berkisar 5–7 detik. Bundle FE produksi yang tersaji pada saat pemeriksaan masih membawa timeout lama `15e3`, meskipun API URL sudah menunjuk Hostinger. Tidak ada migrasi database. | `auth.api.ts`, `axios.ts`, `accounts.service.ts`, `accounts.routes.ts`, `diagnose_auth_readonly.js`, Q11 |
| `ACC-2026-09-16-01` | 16 Sep 2026 | Implemented locally; production smoke pending | Ringkasan berkala dan kehadiran menerima cakupan Global hanya untuk Super Admin; company terpilih tetap membatasi query. Daftar portofolio proyek Global dibuka hanya untuk GET Super Admin, bukan operasi tulis. Company Admin dan role lain tetap wajib memakai company efektif. Super Admin dapat kembali ke pilihan Global dari selector context. Frontend laporan menampilkan kegagalan request secara eksplisit dan menonaktifkan ekspor sampai seluruh sumber utama berhasil dimuat; request gagal tidak lagi muncul sebagai `Rp 0 / 0 proyek`. Tab Executive mensyaratkan sumber Projects dan Finance agar metrik biaya tidak diam-diam dianggap nol saat Finance tidak diizinkan. Tidak memerlukan migrasi database. | `reporting.routes.ts`, `projects.routes.ts`, `ReportingClient.tsx`, `Topbar.tsx`, `report-tab-access.ts`, `reporting-global-scope.unit.ts`, Q11, typecheck/build |
| `ACC-2026-09-15-04` | 15 Sep 2026 | Implemented locally; production smoke pending | Proyeksi berkala dan kehadiran memperlakukan Supervisor seperti Staff: hanya data dirinya, bukan agregat company. Kehadiran mencocokkan `iam_user` ke `master_employee.user_id`, dengan fallback hanya pada `project_member.employee_id` yang sudah eksplisit; tidak mencocokkan UUID user dengan UUID employee. Ringkasan operasional company memerlukan role OM aktif. Kegagalan proyeksi kehadiran ditampilkan pada tab tersebut tanpa menghapus laporan berkala yang berhasil dimuat. | `reporting.routes.ts`, `ReportingClient.tsx`, Q11 |
| `ACC-2026-09-15-03` | 15 Sep 2026 | Implemented locally; production smoke pending | Menu per role kini dibangun oleh kontrak navigasi yang diuji terhadap kontrak route efektif. Staff dan Supervisor melihat `/projects` bila `PROJECTS` aktif; halaman ini tetap memakai row scope assignment/ownership backend. Role tanpa entitlement tidak melihat page terkait, dan delegasi hanya menambah page yang sesuai tanpa membuka `/administration`. Tautan dashboard Company Admin menuju administrasi. Super Admin tidak lagi ditawari mutasi Finance operasional dari UI. | `navigation-contract.ts`, `Sidebar.tsx`, `DashboardClient.tsx`, `capability-contract.ts`, `module-contract.ts`, Q11 |
| `ACC-2026-09-15-02` | 15 Sep 2026 | Implemented locally; production smoke pending | Memisahkan `/administration` dari katalog `/resources`: Super Admin mengaktifkan modul company dan Company Admin mengatur akses anggota pada halaman yang sesuai dengan menu. Fetch daftar company Super Admin memakai scope global eksplisit; pembacaan dan perubahan modul memakai company terpilih, tidak lagi diam-diam tertimpa company di localStorage. `/auth/me` dan perubahan role tidak memakai company header lama; profil menyinkronkan assignment company terbaru. Company Admin mutation di Core memerlukan role aktif. Q11 memeriksa route admin untuk seluruh role dan kontrak header. | `administration/page.tsx`, `AccessAdministration.tsx`, `Sidebar.tsx`, `ResourcesClient.tsx`, `Topbar.tsx`, `AuthContext.tsx`, `auth.api.ts`, `axios.ts`, `core.routes.ts`, `module-contract.ts`, Q11 |
| `ACC-2026-09-15-01` | 15 Sep 2026 | Implemented locally; production smoke pending | Data Explorer memilih katalog pertama yang lolos kontrak akses, menyembunyikan kategori dan tautan modul yang tidak sah, serta menampilkan pesan `403` backend tanpa menganggap request gagal sebagai katalog kosong. Reporting memilih tab yang lolos role dan entitlement; respons lama tidak ditampilkan setelah role/company berubah. Recently Opened hanya mencatat katalog yang berhasil dimuat, memakai satu endpoint canonical Core, dan sidebar menyaring riwayat terhadap akses aktif. Super Admin boleh menyimpan metadata personal melalui POST recent-item/notification-read yang tepat, tetapi mutasi bisnis tetap ditolak. | `ResourcesClient.tsx`, `resource-catalog.ts`, `ReportingClient.tsx`, `report-tab-access.ts`, `Sidebar.tsx`, `module-contract.ts`, `feed.api.ts`, `rbac.middleware.ts`, Q11 |
| `ACC-2026-09-11-02` | 11 Sep 2026 | Implemented and verified locally | Menambahkan journey Timesheet/Lembur khusus Staff: form input, riwayat berpaginasi, ringkasan lembur, refresh setelah submit, dan widget Dashboard. Identitas employee memakai `master_employee.user_id`, lalu fallback hanya ke `project_member.employee_id` yang sudah eksplisit; tidak ada pencocokan nama/username dan tidak ada mass update production. Frontend tidak mengirim employee, rate, amount, atau approval status. | `components/staff/*`, `TasksClient.tsx`, `DashboardClient.tsx`, `project.api.ts`, `projects.routes.ts`, `dashboard.routes.ts`, Q11 |
| `ACC-2026-09-11-01` | 11 Sep 2026 | Implemented and verified locally | Mengunci Daily Task execution sebagai command payload allow-list; menormalisasi alias status lama, menolak status di luar kontrak, mewajibkan alasan saat blocked, dan menampilkan `detail` backend pada UI. Update tidak lagi memberi notifikasi sukses sebelum transaksi berhasil. Idempotency key tetap diwajibkan dan dibuat oleh Axios untuk setiap mutasi. | `project.api.ts`, `TasksClient.tsx`, `ProjectsClient.tsx`, `projects.service.ts`, `projects.routes.ts`, Q11 |
