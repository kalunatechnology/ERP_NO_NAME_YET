...
- **PERSONAL ACCESS LIFECYCLE:** tidak adanya `iam_user_module_access` berarti inherit dari role + entitlement company; row `false/false` adalah explicit block; `true/false` adalah read-only; `true/true` adalah read/write. Write dinormalisasi selalu mengimplikasikan read. Endpoint reset menghapus row override, bukan membuat grant baru, dan seluruh operasi memvalidasi active membership satu company.
